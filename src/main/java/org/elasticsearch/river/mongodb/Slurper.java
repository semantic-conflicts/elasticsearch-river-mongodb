 ...
 */
    ...
    public SlurperException(){
    }
  }
  public static ESLogger logger=ESLoggerFactory.getLogger(Slurper.class.getName());
  public MongoDBRiverDefinition definition;
  public SharedContext context;
  public BasicDBObject findKeys;
  public String gridfsOplogNamespace;
  public String cmdOplogNamespace;
  public ImmutableList<String> oplogOperations=ImmutableList.of(MongoDBRiver.OPLOG_DELETE_OPERATION,MongoDBRiver.OPLOG_UPDATE_ROW_OPERATION,MongoDBRiver.OPLOG_UPDATE_OPERATION,MongoDBRiver.OPLOG_INSERT_OPERATION,MongoDBRiver.OPLOG_COMMAND_OPERATION);
  public Client client;
  public Mongo mongo;
  public DB slurpedDb;
  public DB oplogDb;
  public DBCollection oplogCollection, oplogRefsCollection;
  public AtomicLong totalDocuments=new AtomicLong();
  ...
  public boolean riverHasIndexedFromOplog(){
  ...
  /** 
 * Does an initial sync the same way MongoDB does. https://groups.google.com/ forum/?fromgroups=#!topic/mongodb-user/sOKlhD_E2ns
 ...
 */
  public Timestamp<?> doInitialImport(  DBCollection collection) throws InterruptedException {
  ...
  public BasicDBObject getFilterForInitialImport(  BasicDBObject filter,  String id){
    if (id == null) {
      return filter;
    }
 else {
  ...
  public boolean assignCollections(){
    DB adminDb;
    ...
    if (!definition.getMongoAdminUser().isEmpty() && !definition.getMongoAdminPassword().isEmpty()) {
      ...
      CommandResult cmd=adminDb.authenticateCommand(definition.getMongoAdminUser(),definition.getMongoAdminPassword().toCharArray());
      if (!cmd.ok()) {
    ...
    if (!definition.getMongoLocalUser().isEmpty() && !definition.getMongoLocalPassword().isEmpty() && !oplogDb.isAuthenticated()) {
      ...
      CommandResult cmd=oplogDb.authenticateCommand(definition.getMongoLocalUser(),definition.getMongoLocalPassword().toCharArray());
      if (!cmd.ok()) {
  ...
  public Timestamp<?> processOplogEntry(  final DBObject entry,  final Timestamp<?> startTimestamp) throws InterruptedException {
    flattenOps(entry);
    ...
    if (definition.isMongoGridFS() && namespace.endsWith(MongoDBRiver.GRIDFS_FILES_SUFFIX) && (operation == Operation.INSERT || operation == Operation.UPDATE)) {
      if (objectId == null) {
        throw new NullPointerException(MongoDBRiver.MONGODB_ID_FIELD);
      }
  ...
  @SuppressWarnings("unchecked") public void flattenOps(  DBObject entry){
  ...
  public Object getRefOps(  Object ref){
    DBObject query=new BasicDBObject(MongoDBRiver.MONGODB_ID_FIELD,new BasicDBObject(QueryOperators.GTE,new BasicDBObject(MongoDBRiver.MONGODB_OID_FIELD,ref)));
    DBObject oplog=oplogRefsCollection.findOne(query);
    return oplog == null ? null : oplog.get("ops");
  }
  public void processAdminCommandOplogEntry(  final DBObject entry,  final Timestamp<?> startTimestamp) throws InterruptedException {
    if (logger.isTraceEnabled()) {
  ...
  public String getCollectionFromNamespace(  String namespace){
  ...
  public boolean isValidOplogEntry(  final DBObject entry,  final Timestamp<?> startTimestamp){
    ...
    if (startTimestamp != null) {
      ...
      if (Timestamp.compare(oplogTimestamp,startTimestamp) < 0) {
        logger.error("[Invalid Oplog Entry] - entry timestamp [{}] before startTimestamp [{}]",JSONSerializers.getStrict().serialize(entry),startTimestamp);
        return false;
      }
    }
  ...
  public boolean filterMatch(  DBObject filter,  DBObject object){
  ...
  public DBObject applyFieldFilter(  DBObject object){
  ...
  public String getObjectIdFromOplogEntry(  DBObject entry){
  ...
  public DBCursor oplogCursor(  final Timestamp<?> timestampOverride) throws SlurperException {
  ...
  public void isRiverStale(  DBCursor cursor,  Timestamp<?> time) throws SlurperException {
  ...
  public void addQueryToStream(  final Operation operation,  final Timestamp<?> currentTimestamp,  final DBObject update,  final String collection) throws InterruptedException {
    if (logger.isTraceEnabled()) {
  ...
  public String addInsertToStream(  final Timestamp<?> currentTimestamp,  final DBObject data) throws InterruptedException {
    return addInsertToStream(currentTimestamp,data,definition.getMongoCollection());
  }
  public String addInsertToStream(  final Timestamp<?> currentTimestamp,  final DBObject data,  final String collection) throws InterruptedException {
  ...
  public void addToStream(  final Operation operation,  final Timestamp<?> currentTimestamp,  final DBObject data,  final String collection) throws InterruptedException {
    if (logger.isTraceEnabled()) {
      String dataString=data.toString();
      if (dataString.length() > 400) {
        logger.trace("addToStream - operation [{}], currentTimestamp [{}], data (_id:[{}], serialized length:{}), collection [{}]",operation,currentTimestamp,data.get("_id"),dataString.length(),collection);
      }
 else {
        logger.trace("addToStream - operation [{}], currentTimestamp [{}], data [{}], collection [{}]",operation,currentTimestamp,dataString,collection);
      }
    }
    if (operation == Operation.DROP_DATABASE) {
      logger.info("addToStream - Operation.DROP_DATABASE, currentTimestamp [{}], data [{}], collection [{}]",currentTimestamp,data,collection);
      if (definition.isImportAllCollections()) {
        for (        String name : slurpedDb.getCollectionNames()) {
          logger.info("addToStream - isImportAllCollections - Operation.DROP_DATABASE, currentTimestamp [{}], data [{}], collection [{}]",currentTimestamp,data,name);
